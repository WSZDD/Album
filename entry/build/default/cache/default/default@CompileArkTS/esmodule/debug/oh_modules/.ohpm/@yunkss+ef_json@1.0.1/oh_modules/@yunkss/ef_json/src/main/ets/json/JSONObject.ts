...
import { JSONArrayList } from "@package:pkg_modules/.ohpm/@yunkss+ef_json@1.0.1/pkg_modules/@yunkss/ef_json/src/main/ets/json/JSONArrayList";
        ...
        for (let key of Object.entries(nestedJson)) {
            if (typeof nestedJson[key[0]] === 'object' && nestedJson[key[0]] !== null) {
                if (nestedJson[key[0]] instanceof Date && convertDateStr == true) {
                    ...
                    nestedData[key[0]] =
                        (nestedJson[key[0]] as Array<Record<string, JSONValue>>).map(item => JSONObject.parseNestedObject(item, convertDateStr));
                }
                else {
                    nestedData[key[0]] =
